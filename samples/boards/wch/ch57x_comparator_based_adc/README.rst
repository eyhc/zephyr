...
The divider resistor is connected to the supply voltage and one end of the potentiometer. The
potentiometer wiper is connected to the comparator positive input pin (PA3 or PA7 depending on the
configuration) and the other potentiometer end is connected to ground.

For example, if the supply voltage is 3.3 V, the potentiometer is 10 kΩ and the maximum input
voltage is 850 mV, the divider resistor value is approximately 27 kΩ (the nearest standard value).
...
.. zephyr-app-commands::
   :zephyr-app: samples/boards/wch/ch570_comparator_based_adc
