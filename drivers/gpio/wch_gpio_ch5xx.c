...
#define DT_DRV_COMPAT wch_gpio_ch5xx

#if CONFIG_GPIO_WCH_CH5XX_SUPPORT_PORTB
...
static void gpio_ch5xx_isr(const struct device *dev)
{
	...
	/* mask with enabled interrupts to avoid spurious callbacks */
					      ...
					      enum gpio_int_mode mode, enum gpio_int_trig trigger)
{
	...
	GPIO_REG(16, config->port, INT_IF) = (1 << pin);

	/* GPIO_INT_MODE_DISABLED */

	if (mode == GPIO_INT_MODE_DISABLED) {
		GPIO_REG(16, config->port, INT_EN) &= ~(1 << pin);
		return 0;
	}

	/* GPIO_INT_MODE_ENABLED */

	if (mode == GPIO_INT_MODE_LEVEL) {
		GPIO_REG(16, config->port, INT_MODE) &= ~(1 << pin);
	...
	case GPIO_INT_TRIG_LOW:
		GPIO_REG(16, config->port, INT_EDGE_TYPE) &= ~(1 << pin);
		GPIO_REG(16, config->port, OUT) &= ~(1 << pin);
		break;
...
#define GPIO_CH5XX_IRQ_INIT_PTR                                                                    \
...
#define GPIO_CH5XX_INIT(idx)                                                                       \
	...
	DEVICE_DT_INST_DEFINE(idx, GPIO_CH5XX_IRQ_INIT_PTR, NULL, &gpio_ch5xx_##idx##_data,        \
